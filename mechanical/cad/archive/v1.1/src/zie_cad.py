...
def export_all():
    ...
    for state in ('ACTIVE','SLEEP','SHUTDOWN'):
        a=build_assembly(state,False); path=CAD/f'ZI-E_v1_1_{state}.step'; a.save(str(path)); bboxes[state]=bbox_of_assy(a)
        # glTF for easy viewing
        try: a.save(str(CAD/f'ZI-E_v1_1_{state}.gltf'))
        except Exception as e: print('gltf warning',state,e)
    cut=build_assembly('ACTIVE',True); cut.save(str(CAD/'ZI-E_v1_1_CUTAWAY.step'))
    try: cut.save(str(CAD/'ZI-E_v1_1_CUTAWAY.gltf'))
